...
final class GlWindow {
	...
	void open(uint width, uint height) {
		...
		enforce(glGetNamedBufferParameterivEXT !is null, "EXT_direct_state_access unsupported by this GPU.");
		
		glfwSetCursorPosCallback(window, &cursorCallback);
		
		debug {
			static extern(C) void gl_debug_cb(
				GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
				const(GLchar)* msg, void* userParam
			) nothrow {
				assumeWontThrow(info("OpenGL: ", msg[0..length]));
			}
			
			if(glDebugMessageCallback !is null) {
				glDebugMessageCallback(&gl_debug_cb, null);
				glEnable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
				trace("OpenGL debug logger installed");
			}
		}
	}
	/// ditto
